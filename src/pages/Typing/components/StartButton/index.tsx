...
export default function StartButton({ isLoading }: { isLoading: boolean }) {
  ...
  return (
    <Tooltip content="Shortcut key Enter" className="box-content h-7 w-8 px-6 py-1">
      <div
        ...
        className={`${
          state.isTyping
            ...
            : 'bg-sky-400 shadow-indigo-300 dark:shadow-sky-400/60'
        } ${
      ...
      >
        <button
          className={`${state.isTyping ? 'bg-gray-400  dark:bg-gray-700 dark:hover:bg-gray-500' : 'bg-sky-400'} my-btn-primary w-20 shadow`}
          type="button"
        ...
        {isShowReStartButton && (
          <div className="absolute bottom-0 flex w-20 justify-center" ref={refs.setFloating} {...getFloatingProps()}>
            <button
              className={`${
                state.isTyping ? 'bg-gray-500 dark:bg-gray-700 dark:hover:bg-gray-500 ' : 'bg-sky-300 '
